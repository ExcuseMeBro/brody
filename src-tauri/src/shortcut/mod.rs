...
//! - `brody_keys`: Uses the keyboard library for more control
//!
...
// Note: Commands are accessed via shortcut::brody_keys:: in lib.rs
...
pub fn init_shortcuts(app: &AppHandle) {
    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::BrodyKeys => {
            if let Err(e) = brody_keys::init_shortcuts(app) {
                error!("Failed to initialize Brody keys shortcuts: {}", e);
                ...
                // Update settings to persist the fallback so we don't retry Brody keys on next launch
...
pub fn register_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::BrodyKeys => brody_keys::register_cancel_shortcut(app),
    }
}
...
pub fn unregister_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::BrodyKeys => brody_keys::unregister_cancel_shortcut(app),
    }
}
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::BrodyKeys => brody_keys::register_shortcut(app, binding),
    }
}
...
pub fn unregister_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::BrodyKeys => brody_keys::unregister_shortcut(app, binding),
    }
}
...
pub fn change_keyboard_implementation_setting(
    ...
    // Initialize new implementation if needed (BrodyKeys needs state)
    if new_impl == KeyboardImplementation::BrodyKeys {
        if initialize_brody_keys_with_rollback(&app)? {
...
pub fn get_keyboard_implementation(app: AppHandle) -> String {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::BrodyKeys => "brody_keys".to_string(),
    }
}
...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::BrodyKeys => brody_keys::validate_shortcut(raw),
    }
}
...
/// Initialize BrodyKeys if not already initialized, with rollback on failure
fn initialize_brody_keys_with_rollback(app: &AppHandle) -> Result<bool, String> {
    if app.try_state::<brody_keys::BrodyKeysState>().is_some() {
        return Ok(false); // Already initialized, caller should continue
    }

    if let Err(e) = brody_keys::init_shortcuts(app) {
        error!("Failed to initialize BrodyKeys: {}", e);
        // Rollback to Tauri
        let mut settings = settings::get_settings(app);
        settings.keyboard_implementation = KeyboardImplementation::Tauri;
        settings::write_settings(app, settings);
        tauri_impl::init_shortcuts(app);
        return Err(format!(
            "Failed to initialize BrodyKeys: {}. Reverted to Tauri.",
            e
        ));
    }
