...
interface BrodyKeysShortcutInputProps {
...
export const BrodyKeysShortcutInput: React.FC<BrodyKeysShortcutInputProps> = ({
  ...
  const cancelRecording = useCallback(async () => {
    ...
    await commands.stopBrodyKeysRecording().catch(console.error);
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<BrodyKeysEvent>(
        "brody-keys-event",
        async (event) => {
          if (cleanup) return;
    ...
    return () => {
      ...
      commands.stopBrodyKeysRecording().catch(console.error);
    };
  }, [
    isRecording,
    shortcutId,
  ...
  const startRecording = async () => {
    ...
    try {
      await commands.startBrodyKeysRecording(shortcutId);
      setIsRecording(true);
      setCurrentKeys("");
      currentKeysRef.current = "";
    } catch (error) {
